...
function shufflePosts(posts: SitePost[]) {
  const copy = [...posts]
...
export default async function HomePage() {
  ...
  const primaryPosts = shufflePosts(uniquePosts(taskFeed.find(({ task }) => task.key === primaryTask)?.posts || taskFeed.flatMap(({ posts }) => posts))).slice(0, 30)
