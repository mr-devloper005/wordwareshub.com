...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  if (task === 'article') {
    const lead = posts[0]
    const editorialGrid = posts.slice(1)
    return (
      <EditableSiteShell>
        <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
          <section className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            <div className="rounded-[2.2rem] border border-[var(--editable-border)] bg-white/70 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur sm:p-10">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full bg-[var(--archive-text)] px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-bg)]">Editorial Edition</span>
                <span className="rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.2em]">{categoryLabel}</span>
              </div>
              <h1 className="mt-6 max-w-5xl text-4xl font-black leading-[0.95] tracking-[-0.07em] sm:text-6xl">Featured reporting, sharp commentary, and long-form reads.</h1>
              <p className="mt-5 max-w-3xl text-base leading-8 opacity-70">A redesigned article desk with stronger visual hierarchy and cleaner reading pathways.</p>
              <div className="mt-8 grid gap-3 sm:grid-cols-[1fr_auto]">
                <form action={basePath} className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
                  <select name="category" defaultValue={category} className="h-12 rounded-full border border-[var(--editable-border)] bg-white px-5 text-sm font-bold outline-none">
                    <option value="all">All categories</option>
                    {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
                  </select>
                  <button className="h-12 rounded-full bg-[var(--archive-text)] px-6 text-sm font-black text-[var(--archive-bg)]">Apply filter</button>
                </form>
                <Link href="/search" className="inline-flex h-12 items-center justify-center rounded-full border border-[var(--editable-border)] px-6 text-sm font-black">Search</Link>
              </div>
            </div>
          </section>
          ...
          <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
            {lead ? (
              <Link href={`${basePath}/${lead.slug}`} className="group grid overflow-hidden rounded-[2.2rem] border border-[var(--editable-border)] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.10)] transition duration-300 hover:-translate-y-1 md:grid-cols-[1.05fr_0.95fr]">
                <div className="relative min-h-[320px] bg-[var(--archive-bg)] md:min-h-[460px]">
                  <img src={getImage(lead)} alt={lead.title} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                </div>
                <div className="p-7 sm:p-10">
                  <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--archive-accent)]">{getCategory(lead, 'Article')}</p>
                  <h2 className="mt-4 text-3xl font-black leading-tight tracking-[-0.05em] sm:text-4xl">{lead.title}</h2>
                  <p className="mt-5 line-clamp-5 text-sm leading-8 opacity-70">{getSummary(lead)}</p>
                  <span className="mt-7 inline-flex items-center gap-2 rounded-full bg-[var(--archive-text)] px-5 py-3 text-sm font-black text-[var(--archive-bg)]">Read feature <ArrowRight className="h-4 w-4" /></span>
                </div>
              </Link>
            ) : null}

            {editorialGrid.length ? (
              <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {editorialGrid.map((post, index) => <ArticleArchiveCard key={post.id || post.slug} post={post} href={`${basePath}/${post.slug}`} index={index + 1} />)}
              </div>
            ) : (
              <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/60 p-10 text-center">
