...
export function EditableNavbar() {
  ...
  const navVars = {
    '--editable-nav-bg': 'var(--slot4-page-bg)',
    '--editable-nav-text': 'var(--slot4-page-text)',
    '--editable-nav-active': 'var(--slot4-dark-bg)',
    '--editable-nav-active-text': 'var(--slot4-dark-text)',
    '--editable-cta-bg': 'var(--slot4-dark-bg)',
    '--editable-cta-text': 'var(--slot4-dark-text)',
    '--editable-search-bg': 'var(--slot4-surface-bg)',
    '--editable-border': 'color-mix(in oklab, var(--slot4-page-text) 16%, transparent)',
    ...
    <header style={navVars} className="sticky top-0 z-50 border-b border-[var(--editable-border)] bg-[var(--editable-nav-bg)]/92 text-[var(--editable-nav-text)] backdrop-blur-2xl">
      ...
      <nav className="mx-auto flex min-h-[86px] w-full max-w-[var(--editable-container)] items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          <img src="/favicon.png?v=20260413" alt={SITE_CONFIG.name} className="h-16 w-16 object-contain transition-transform group-hover:scale-105" />
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {navItems.slice(0, 4).map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`rounded-full px-4 py-2 text-sm font-black transition ${active ? 'bg-[var(--editable-nav-active)] text-[var(--editable-nav-active-text)] shadow-[0_8px_20px_rgba(0,0,0,0.14)]' : 'hover:bg-black/5'}`}>
                {item.label}
              </Link>
            )
          })}
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {session ? (
            <>
              <Link href="/contact" className="hidden items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-4 py-2.5 text-sm font-black transition hover:-translate-y-0.5 md:inline-flex">Create Article</Link>
              <button type="button" onClick={logout} className="hidden items-center gap-2 rounded-full bg-[var(--editable-cta-bg)] px-4 py-2.5 text-sm font-black text-[var(--editable-cta-text)] shadow-sm transition hover:-translate-y-0.5 sm:inline-flex">Sign out</button>
            </>
          ) : (
            <>
              ...
              <Link href="/signup" className="hidden items-center gap-2 rounded-full bg-[var(--editable-cta-bg)] px-4 py-2.5 text-sm font-black text-[var(--editable-cta-text)] shadow-sm transition hover:-translate-y-0.5 sm:inline-flex"><UserPlus className="h-4 w-4" /> Sign up</Link>
            </>
          )}
      ...
      {open ? (
        <div className="border-t border-[var(--editable-border)] bg-[var(--editable-nav-bg)] px-4 py-4 lg:hidden">
          ...
          <div className="grid gap-2">
            {[{ label: 'Home', href: '/' }, ...navItems, ...(session ? [{ label: 'Create Article', href: '/contact' }] : []), { label: 'Contact', href: '/contact' }].map((item) => (
            ...
            {session ? <button type="button" onClick={() => { logout(); setOpen(false) }} className="rounded-2xl border border-[var(--editable-border)] bg-[var(--editable-cta-bg)] px-4 py-3 text-sm font-black text-[var(--editable-cta-text)]">Sign out</button> : null}
          </div>
        </div>
      ) : null}
    </header>
